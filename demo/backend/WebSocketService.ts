...
import { WSMessage, ClassroomState } from '../types';
...
export class WebSocketService {
  ...
  private courseRooms: Map<string, Set<string>> = new Map(); // courseId -> socketIds
  ...
  private setupEventHandlers(): void {
    this.io.on('connection', (socket: Socket) => {
      ...
      socket.on('subscribe:course', (data: { courseId: string }) => {
        this.handleSubscribeCourse(socket, data.courseId);
      });

      // Unsubscribe from course
      socket.on('unsubscribe:course', (data: { courseId: string }) => {
        this.handleUnsubscribeCourse(socket, data.courseId);
      });

      // Request classroom state
      socket.on('get:classroomState', async (data: { courseId: string }, callback) => {
        // This will be handled by the registration service
  ...
  private handleAuthenticate(socket: Socket, studentId: string): void {
  ...
  private handleSubscribeCourse(socket: Socket, courseId: string): void {
    const client = this.clients.get(socket.id);
    if (!client) return;
  ...
  private handleUnsubscribeCourse(socket: Socket, courseId: string): void {
    const client = this.clients.get(socket.id);
    if (!client) return;
   ...
   */
  broadcastToCourse(courseId: string, message: WSMessage): void {
   ...
   */
  broadcastClassroomState(courseId: string, state: ClassroomState): void {
   ...
   */
  sendToStudent(studentId: string, message: WSMessage): void {
  ...
  broadcastWaitlistUpdate(
    courseId: string,
    waitlistSize: number,
    topCandidates: Array<{ position: number; studentId: string; score: number }>
  ): void {
    this.broadcastToCourse(courseId, {
   ...
   */
  broadcastBookingStatusChange(courseId: string, newStatus: string): void {
    this.broadcastToCourse(courseId, {
   ...
   */
  getCourseViewerCount(courseId: string): number {
